...
export class UserService {
  constructor(private jwtService: JwtService, @InjectModel(User.name) private userModel: Model<User>, private readonly filesService: FilesService) { }
  ...
  async getSystemAdmin() {
    return await this.userModel.findById('655d0603b24b74872de2e0ca');
  }
  ...
  async signIn(user: User): Promise<any> {
    const u = await this.getUser(user.name, user.password);
    if (u == undefined || !u.active) {
      throw new UnauthorizedException();
    }
    let payload;
    if (u._id.equals((await this.getSystemAdmin())._id)) {
      payload = { username: user.name, password: user.password, roles: ['system_admin', 'admin', 'user'] };
    }
    else {
      payload = { username: user.name, password: user.password, roles: ['user'] };
    }
  ...
  async update(id: ObjectId, user: User, auth: string, image: Express.Multer.File) {
    ...
    // console.log(await this.filesService.upload(image));
    try {
  ...
  async forgerPassword(_user: User) {
